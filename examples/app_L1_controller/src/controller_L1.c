...
PARAM_ADD(PARAM_FLOAT, kv_x, &kv_x)
PARAM_ADD(PARAM_FLOAT, kv_y, &kv_y)
...
PARAM_ADD(PARAM_FLOAT, ko_x, &ko_x)
PARAM_ADD(PARAM_FLOAT, ko_y, &ko_y)
...
// baseline inputs, PWM
LOG_ADD(LOG_FLOAT, bsln_pwm_thrust, &bsln_pwm_thrust)
LOG_ADD(LOG_FLOAT, bsln_pwm_Mx, &bsln_pwm_moments.x)
LOG_ADD(LOG_FLOAT, bsln_pwm_My, &bsln_pwm_moments.y)
LOG_ADD(LOG_FLOAT, bsln_pwm_Mz, &bsln_pwm_moments.z)

// baseline inputs, SI
LOG_ADD(LOG_FLOAT, bsln_SI_thrust, &bsln_SI_thrust)
LOG_ADD(LOG_FLOAT, bsln_SI_Mx, &bsln_SI_moments.x)
LOG_ADD(LOG_FLOAT, bsln_SI_My, &bsln_SI_moments.y)
LOG_ADD(LOG_FLOAT, bsln_SI_Mz, &bsln_SI_moments.z)

// prediction error
LOG_ADD(LOG_FLOAT, v_tilda_x, &v_tilda.x)
LOG_ADD(LOG_FLOAT, v_tilda_y, &v_tilda.y)
LOG_ADD(LOG_FLOAT, v_tilda_z, &v_tilda.z)
LOG_ADD(LOG_FLOAT, w_tilda_x, &w_tilda.x)
LOG_ADD(LOG_FLOAT, w_tilda_y, &w_tilda.y)
LOG_ADD(LOG_FLOAT, w_tilda_z, &w_tilda.z)

// // uncertainties
// LOG_ADD(LOG_FLOAT, sigma_fx, &sigma_fx)
// LOG_ADD(LOG_FLOAT, sigma_fy, &sigma_fy)
LOG_ADD(LOG_FLOAT, sigma_fz, &sigma_f.z)
LOG_ADD(LOG_FLOAT, sigma_Mx, &sigma_m.x)
LOG_ADD(LOG_FLOAT, sigma_My, &sigma_m.y)
LOG_ADD(LOG_FLOAT, sigma_Mz, &sigma_m.z)

LOG_ADD(LOG_FLOAT, sigma_inj_fz, &sigma_inj_fz)


// adjusted inputs, SI
LOG_ADD(LOG_FLOAT, adj_SI_thrust, &adj_SI_thrust)
LOG_ADD(LOG_FLOAT, adj_SI_Mx, &adj_SI_moments.x)
LOG_ADD(LOG_FLOAT, adj_SI_My, &adj_SI_moments.y)
LOG_ADD(LOG_FLOAT, adj_SI_Mz, &adj_SI_moments.z)


// control inputs, SI
LOG_ADD(LOG_FLOAT, cmd_SI_thrust, &cmd_SI_thrust)
LOG_ADD(LOG_FLOAT, cmd_SI_Mx, &cmd_SI_moments.x)
LOG_ADD(LOG_FLOAT, cmd_SI_My, &cmd_SI_moments.y)
LOG_ADD(LOG_FLOAT, cmd_SI_Mz, &cmd_SI_moments.z)

// control inputs, PWM
LOG_ADD(LOG_FLOAT, cmd_thrust, &cmd_thrust)
LOG_ADD(LOG_FLOAT, cmd_Mx, &cmd_roll)
LOG_ADD(LOG_FLOAT, cmd_My, &cmd_pitch)
LOG_ADD(LOG_FLOAT, cmd_Mz, &cmd_yaw)

// state estimates
LOG_ADD(LOG_FLOAT, v_hat_x, &v_hat.x)
LOG_ADD(LOG_FLOAT, v_hat_y, &v_hat.y)
LOG_ADD(LOG_FLOAT, v_hat_z, &v_hat.z)
LOG_ADD(LOG_FLOAT, w_hat_x, &w_hat.x)
LOG_ADD(LOG_FLOAT, w_hat_y, &w_hat.y)
LOG_ADD(LOG_FLOAT, w_hat_z, &w_hat.z)

// low pass filter
LOG_ADD(LOG_FLOAT, filtd1_fz, &sigma_fz_filt1)
LOG_ADD(LOG_FLOAT, filtd1_mx, &sigma_m_filt1.x)
LOG_ADD(LOG_FLOAT, filtd1_my, &sigma_m_filt1.y)
LOG_ADD(LOG_FLOAT, filtd1_mz, &sigma_m_filt1.z)
LOG_ADD(LOG_FLOAT, filtd_fz, &sigma_fz_filt2)
LOG_ADD(LOG_FLOAT, filtd_mx, &sigma_m_filt2.x)
...
LOG_ADD(LOG_FLOAT, w_f1, &w_f1)
LOG_ADD(LOG_FLOAT, w_f2, &w_f2)
LOG_ADD(LOG_FLOAT, w_m1, &w_m1)
